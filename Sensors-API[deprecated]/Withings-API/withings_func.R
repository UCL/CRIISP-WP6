...
###############################################################################
####  BEGIN OF CSV FUNCTIONS
###############################################################################


###############################################################################
###############################################################################
####  BEGIN OF CSV EXPAND FUNCTION
###############################################################################
###############################################################################

# Function to expand sensor data
expand_csv_data <- function(data) {
  data <- data %>%
    mutate(
      duration = str_remove_all(duration, "\\[|\\]|\\s"), # Remove brackets and whitespace
      value = str_remove_all(value, "\\[|\\]|\\s") # Remove brackets and whitespace
    ) %>%
    mutate(
      duration = str_split(duration, ","), # Split by comma
      value = str_split(value, ","),
      duration = lapply(duration, as.numeric), # Convert to numeric
      value = lapply(value, as.numeric)
    )
  
  # Convert durations to cumulative durations
  data <- data %>%
    mutate(duration = lapply(duration, cumsum)) %>%
    unnest(c(duration, value)) %>%
    mutate(
      # Create timestamps based on the cumulative duration
