...
Participant_id<-"credentials/P_00"
...
# this should read client_id client_secret refresh_token as environment variables
```
...
credentials<-readLines(Participant_id)

# Find the line that contains 'refresh_token'
line_index <- grep("refresh_token", credentials)

credentials[line_index] <- paste0('refresh_token <- "', refresh_token, '"')

# Write the updated credentials to config.r 
writeLines(credentials, Participant_id)
